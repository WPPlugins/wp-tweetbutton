...
<tr><td>Twitter Handle</td><td><input type="text" name="tweetbutton_name" value="<?php echo get_option('tweetbutton_name'); ?>" size="40"><span class="description"> Do not use @ or a link to the profile</span></td><br/></tr>
...
if(get_option('tweetbutton_syntax')=="via"){$text=$tweet_text;}
else if(get_option('tweetbutton_name')!=false) $text="RT @".get_option('tweetbutton_name')." ".$tweet_text; else $text=$tweet_text;
$link=get_permalink($post->ID);
$tweetcode= '<div class="tweet_button" style="'.$css.'"><a href="http://twitter.com/share" rel="nofollow" class="twitter-share-button" data-url="'.$link.'" data-text="'.$text.'" data-count="'.$style.'" data-lang="'.$lang.'" ';
if(get_option('tweetbutton_syntax')=="via") $tweetcode=$tweetcode.'data-via="'.$twitterhandle.'" ';  
if($tweetrel==false&&get_option('tweetbutton_syntax')=="via") $tweetcode=$tweetcode.'></a></div>';
else if(get_option('tweetbutton_syntax')=="rt"&&$twitterhandle==false)
$tweetcode=$tweetcode.' data-related="'.$tweetrel.':'.$tweetreldesc.'"></a></div>';
else if(get_option('tweetbutton_syntax')=="rt"&&$twitterhandle!=false)
$tweetcode=$tweetcode.' data-related="'.$twitterhandle.'"></a></div>';
else $tweetcode=$tweetcode.'></a></div>';
return $tweetcode;
}
